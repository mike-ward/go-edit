...
Phases 0, 1, 1.2 (File I/O), 1.5 (Extension substrate), 2 (Selection + clipboard), 3 (Undo/redo), 5 (Multi-cursor), and 6 (Search/replace) are committed. Phase 6 adds a custom-drawn find bar (literal + regex, case toggle), find next/prev with match highlighting, and replace/replace-all (single undo entry). Search state lives in `editorState.Search`; input routing checks `Search.Active` before normal keymap dispatch. Golden-frame tests are deferred until an upstream `DrawContext.Inspect()` accessor exists. The plain-`[]byte` line store will be replaced with a per-line gap buffer once bench pressure justifies it. See `ROADMAP.md` for the full phase list and open questions.
