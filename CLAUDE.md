...
Phases 0, 1, 1.2 (File I/O), 1.5 (Extension substrate), 2 (Selection + clipboard), 3 (Undo/redo), and 5 (Multi-cursor) are committed. Phase 5 adds `[]CursorState` on `editorState`, per-cursor action dispatch with `PostEditFunc`-based position adjustment, sort/merge, Alt-click add cursor, Ctrl+D add next match, Escape collapse, and multi-cursor clipboard. Golden-frame tests are deferred until an upstream `DrawContext.Inspect()` accessor exists. The plain-`[]byte` line store will be replaced with a per-line gap buffer once bench pressure justifies it. See `ROADMAP.md` for the full phase list and open questions.
